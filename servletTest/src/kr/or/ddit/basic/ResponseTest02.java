...
public class ResponseTest02 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 이 때 사용하는 객체는 RequestDispatcher객체이다.
		
		// 이용되는 페이지로 값을 넘기려면 request객체의 setAttribute()메서드로 데이터를 셋팅하여 보내고 
		// 받는 쪽에서는 getAttribute()메서드로 데이터를 읽어온다.
		//		형식) setAttribute("key값", value값);		//key값은 문자열, value값은 모든 데이터를 다 사용할 수 있다.

		request.setAttribute("tel", "010-4724-5496");
		
		// getRequestDispatchar("forward할 문서명")
